...
w1_gcn = tf.get_variable(name = "w1", shape = [3*no_filters  ,no_classes], regularizer = regularizer, 
                                initializer=initializer,dtype=tf.float32) 
b1_gcn = tf.get_variable(name = "b1",shape = [1,no_classes], initializer=initializer,regularizer = regularizer,dtype=tf.float32)



layer_embedding_gcn = tf.reshape(final_input,[ tf.shape(x)[0] , tf.shape(x)[1] *60,1 ])

window_gcn= tf.Variable(tf.random_normal([filter_size,1,no_filters])) # filter_size X 1 X no_filters(3D)


layer_zeros_gcn=tf.zeros([ tf.shape(x)[0],60,1])

layer_embedding_gcn=tf.concat([layer_zeros,layer_embedding],axis=1)

layer_embedding_gcn=tf.concat([layer_embedding,layer_zeros],axis=1)

conv_layer_gcn  =  tf.nn.conv1d(layer_embedding, window , stride=1*60 , padding='VALID' )# no of examples X no of words X no_filters(3D)


alpha1_gcn= tf.get_variable(name = "alpha1", shape = [1,no_classes], regularizer = regularizer, 
                                initializer=initializer,dtype=tf.float32) 

alpha2_gcn= tf.get_variable(name = "alpha2", shape = [1,no_classes], regularizer = regularizer, 
                                initializer=initializer,dtype=tf.float32) 
...
alpha1= tf.get_variable(name = "alpha1", shape = [1,no_classes], regularizer = regularizer, 
                                initializer=initializer,dtype=tf.float32) 

alpha2= tf.get_variable(name = "alpha2", shape = [1,no_classes], regularizer = regularizer, 
                                initializer=initializer,dtype=tf.float32) 
...
max_pool=tf.nn.dropout(max_pool,keep_prob=prob)
...
Dense_weights = tf.get_variable(name = "Dense_weights", shape = [2*3*num_neurons,no_classes], regularizer = regularizer, 
                                initializer=initializer) 
Dense_bias = tf.get_variable(name = "Dense_bias",shape = [no_classes], initializer=initializer)
...
with tf.Session() as sess:
    ...
    layer_dnn1 = tf.add(tf.matmul(max_pool,w1),b1)
    #logits_dnn1=layer_dnn1
    
    layer_dnn2 = tf.add(tf.matmul(att_outputs,Dense_weights),Dense_bias)
    #logits_dnn2=layer_dnn2
    
    logits = layer_dnn1*alpha1+ layer_dnn2*alpha2 # 
    ...
    for epoch in range(epochs):
        ...
        if(epoch%5==0):
            ...
            temp  = sklearn.metrics.f1_score(np.argmax(one_hot_label_test,1)[:2700],y_pred,average='weighted')
            f1_test.append(temp)
